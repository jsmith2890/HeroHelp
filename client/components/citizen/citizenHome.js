...
class CitizenHome extends Component {
  ...
  preRenderMapMath() {
    ...
    if (
      ...
      let deltaMapLat = Math.abs(
        this.initialHeroCoords.lat - incidentCoords.lat,
      );
      let deltaMapLon = Math.abs(
        this.initialHeroCoords.lon - incidentCoords.lon,
      );
      ...
      if (deltaDeltaLat < 0.4 && deltaDeltaLon < 0.4) {
        //100-60=>40%
        this.initialHeroCoords = { lat: hero.lat, lon: hero.lon };
      }
    }
      ...
      <Fragment>
        ...
        {status === CitizenState.KNOWS_HERO_ENROUTE && (
          <HeroEnroute
            ...
            initialHeroCoords={this.initialHeroCoords}
          />
        )}
        {status === CitizenState.KNOWS_HERO_ON_SITE && (
          <HeroOnSite hero={hero} />
        )}
      </Fragment>
    );
  }
